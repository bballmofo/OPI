...
if init_env:
  ...
  use_other_env = False
  other_env_exists = os.path.isfile('../brc6699_index/.env')
  if other_env_exists:
    res = input(".env on brc6699_index already exists, do you want to use values from there? (y/n) ")
    if res == 'y':
      use_other_env = True
  if use_other_env:
  ...
  res = input("brc6699 Postgres DB use SSL (Default: true) may need to be set to false on Windows machines: ")
  if res != '':
    DB_SSL = res
  res = input("brc6699 Postgres DB max connections (Default: 10): ")
  if res != '':
