...
if init_env:
  ...
  ORD_SERVER_URL="http://localhost"
  REPORT_RETRIES="10"
  REPORT_NAME="opi_brc6699_index"
  CREATE_EXTRA_TABLES="true"
  print("Initialising .env file")
  print("leave blank to use default values")
  use_other_env = False
  other_env_exists = os.path.isfile('../brc6699_api/.env')
  if other_env_exists:
    res = input(".env on brc6699_api already exists, do you want to use values from there? (y/n) ")
    if res == 'y':
      use_other_env = True
  if use_other_env:
    env = dotenv_values(dotenv_path='../brc6699_api/.env')
    ...
    res = input("brc6699 Postgres DB username (Default: postgres): ")
    if res != '':
      DB_USER = res
    res = input("brc6699 Postgres DB host (Default: localhost) leave default if postgres is installed on the same machine: ")
    if res != '':
      DB_HOST = res
    res = input("brc6699 Postgres DB port (Default: 5432): ")
    if res != '':
      DB_PORT = res
    res = input("brc6699 Postgres DB name (Default: postgres) leave default if no new dbs are created: ")
    if res != '':
      DB_DATABASE = res
    res = stdiomask.getpass("brc6699 Postgres DB password: ")
    DB_PASSWD = res
  use_main_env = False
  ...
  res = input("ORD_SERVER_URL (Default: http://localhost): ")
  if res != '':
    ORD_SERVER_URL = res
  res = input("Create extra tables for faster queries (Default: true) set to true for creating brc6699_current_balances and brc6699_unused_tx_inscrs tables: ")
  if res != '':
...
db_exists = False
try:
  cur.execute('select count(*) from brc6699_block_hashes;')
