...
try:
  ...
  print("Error connecting to grc20 database, please check .env file")
  traceback.print_exc()
  sys.exit(1)
...
try:
  ...
  row = cur.fetchone()
  if row:
    grc20_min_height = row[0]
    grc20_max_height = row[1]
except:
  print("Error getting grc20 block height info")
  traceback.print_exc()
  sys.exit(1)
...
if main_min_height > 767430:
  print("main_min_height is greater than 767430, please check if you are using the correct .env file and rerun the main & grc20 indexer from start (run reset.py first)")
  sys.exit(1)

if grc20_min_height != 767430:
  print("grc20_min_height is not equal to 767430, please check if you are using the correct .env file and rerun the grc20 indexer from start (run reset.py first)")
  sys.exit(1)
...
def check_block_hashes(height):
  ...
  cur.execute('''select bceh.block_event_hash, bceh.cumulative_event_hash, bbh.block_hash 
    from grc20_cumulative_event_hashes bceh 
    left join grc20_block_hashes bbh on bbh.block_height = bceh.block_height
...
current_max_height = grc20_max_height
if check_block_hashes(current_max_height):
  print("grc20 block hashes are correct")
  sys.exit(0)

if not check_block_hashes(current_min_height):
  print("grc20 block hashes are incorrect from the start, please check if you are using the correct .env file and rerun the grc20 indexer from start (run reset.py first)")
  sys.exit(1)

while True:
  if current_max_height - current_min_height <= 1:
    print("grc20 block hashes are incorrect starting at block (" + str(current_max_height) + "), please check if you are using the correct .env file and rerun the grc20 indexer from start (run reset.py first)")
    sys.exit(1)
